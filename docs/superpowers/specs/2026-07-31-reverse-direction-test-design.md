...
### 3.1 `TEST_PHASE_BEGIN` 载荷 12 → 16 字节

追加 `spread` 字(0 = 单端口,1 = 按数据端口轮询)。老 responder 的检查是
`pl_len < 12` 才拒,收到 16 字节照常读前 12 字节。
...
### 3.3 `TEST_HELLO_ACK` 接受路径 4 → 8 字节(能力位)

| 路径 | 布局 | 老 prober 行为 |
|---|---|---|
| 接受 | `reject=0` + `caps` | 只读偏移 0 判 `!=0`,后面不看 → 正常 |
| 拒绝 | `reject` + 原因串(**不变**) | 从偏移 4 读原因串 → 正常 |

能力位只出现在接受路径,而接受路径没有原因串,故新旧组合都不会把能力位当作
文本打印。`caps` 位 0 = 支持反向阶段。
...
持有 `-k` 的对端本就能驱动 responder,但不该能让它以任意速率无限量发送;目的
地址一项已被 source pinning 锁死,反向探测只发给完成过 MAC 认证握手的 peer。
