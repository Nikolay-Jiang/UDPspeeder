...
off 20 : pkt_size  (本阶段探测包填充长度;见 3.5.2)
```
...
### 3.3.1 `TEST_PHASE_ACK` 在 `dir=1` 阶段携带 4 字节载荷

```
off 0 : ports_n   本阶段 responder 实际使用的端口数(可能 < 请求数)
```

老 prober 完全不看 `PHASE_ACK` 载荷,故这是零成本追加。缺失/短载荷解码为 0,含义
是"对端没报",报告须据此说明端口数未知,而不是拿请求数顶替(见 4.3)。
...
#### 3.5.2 探测包长必须随 `PHASE_BEGIN` 上线

反向探测原先按 **responder 自己**的 `--test-pkt-size` 填充,而报告表头的"包长"
和下行带宽列用的是 **prober** 的值。只在 prober 侧传 `--test-pkt-size 400`(选
项表并未标注它需要两端一致,文档里的 responder 启动命令也不带任何 test 选项),
下行实际按 1200B 测量却按 400B 报告——包长在 MTU 受限和被限速的链路上会实质改变
丢包率。

`PHASE_BEGIN` 追加 `pkt_size` 字(见 3.1 的追加式兼容规则),responder 收到后先
夹到自己的 `TEST_PKT_SIZE_MIN/MAX` 再用(对端可控输入),`0` 表示旧对端没发、回
退到本地配置。这样"必须两端一致"的选项列表不会变长。
